...
- テスト: `cargo test`（23 件）。プレースホルダ展開・設定マージ・TOML 解析（windows /
  unknown field 拒否）・worktree porcelain 解析・gone 抽出・session_name・
  worktree_base 解決・expand_tilde 等の純粋ロジックを網羅。
